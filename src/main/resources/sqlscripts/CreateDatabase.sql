...
-- Dumping structure for table uruappdb.catalog_entity
CREATE TABLE IF NOT EXISTS `catalog_entity` (
  `ENTITY_ID` bigint(15) NOT NULL AUTO_INCREMENT COMMENT 'PK—a unique, system-generated number used as a record identifier',
  `ENTITY_NAME` varchar(200) NOT NULL COMMENT 'The name of the entity.',
  `ENTITY_TYPE` varchar(100) NOT NULL COMMENT 'The type of entity (e.g., User, Group, etc.)',
...
) ENGINE=InnoDB DEFAULT CHARSET=latin1 COMMENT='This table stores unique identification numbers for configurable entities.';
...
CREATE TABLE IF NOT EXISTS `catalog_property_key` (
  ...
  `CREATION_DATE` date NOT NULL COMMENT 'The time and date the record was created.',
  `MODIFIED_DATE` date NOT NULL COMMENT 'The time and date the record was last modified.',
  `PROPERTY_SET_NAME` varchar(100) NOT NULL COMMENT 'The name of the property set (formerly the SCOPE_NAME from WLCS_PROP_ID).',
...
CREATE TABLE IF NOT EXISTS `catalog_property_value` (
  ...
  `ENTITY_ID` bigint(15) NOT NULL COMMENT 'A system-generated value and foreign key to the ENTITY column.',
...
CREATE TABLE IF NOT EXISTS `discount_association` (
  ...
  `CUSTOMER_ID` int(15) NOT NULL COMMENT 'FK—foreign key to the DISCOUNT_SET table.',
  ...
  CONSTRAINT `FK_discount_association_discount_set` FOREIGN KEY (`CUSTOMER_ID`) REFERENCES `discount_set` (`DISCOUNT_SET_ID`),
...
CREATE TABLE IF NOT EXISTS `ecom_cart` (
  ...
  `SKU` varchar(20) DEFAULT NULL COMMENT 'A unique identifier (the Stock Keeping Unit or SKU) for a product item.',
  PRIMARY KEY (`ID`)
...
CREATE TABLE IF NOT EXISTS `ecom_order_line` (
  ...
  `PRODUCT_ID` varchar(40) DEFAULT NULL COMMENT 'An identification number for the item in the shopping cart.',
...
CREATE TABLE IF NOT EXISTS `ecom_product` (
  `SKU` varchar(40) NOT NULL COMMENT 'A unique identifier (the "Stock Keeping Unit," or SKU) for a product item. This field is the table''s primary key and cannot be NULL. All other fields in the WLCS_PRODUCT table can be NULL.',
...
-- Dumping structure for table uruappdb.ecom_product_category
CREATE TABLE IF NOT EXISTS `ecom_product_category` (
  `ID` bigint(15) NOT NULL,
  `SKU` varchar(40) NOT NULL COMMENT 'A unique identifier (the "Stock Keeping Unit," or SKU) for an item.',
  `CATEGORY_ID` varchar(20) NOT NULL COMMENT 'A unique identifier for a category.',
  PRIMARY KEY (`ID`),
  UNIQUE KEY `SKU` (`SKU`),
  UNIQUE KEY `CATEGORY_ID` (`CATEGORY_ID`),
  CONSTRAINT `FK_ecom_product_category_ecom_product` FOREIGN KEY (`SKU`) REFERENCES `ecom_product` (`SKU`),
  CONSTRAINT `FK_ecom_product_category_ecom_category` FOREIGN KEY (`CATEGORY_ID`) REFERENCES `ecom_category` (`CATEGORY_ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1 COMMENT='This table is used to join categories and items.';
...
CREATE TABLE IF NOT EXISTS `ecom_product_keyword` (
  ...
  `KEYWORD` varchar(30) NOT NULL COMMENT 'Contains a keyword that you associate with the product item assigned to the unique SKU.',
  `SKU` varchar(40) NOT NULL COMMENT 'A unique identifier (the "Stock Keeping Unit," or SKU) for an item.',
  PRIMARY KEY (`ID`),
  KEY `FK_ecom_product_keyword_ecom_product` (`SKU`),
  CONSTRAINT `FK_ecom_product_keyword_ecom_product` FOREIGN KEY (`SKU`) REFERENCES `ecom_product` (`SKU`)
